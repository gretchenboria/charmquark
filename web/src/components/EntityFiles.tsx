...
// Vault files attached to an entity (recordings, documents, images). Upload reads the file in the
...
export function EntityFiles({
  ...
  const load = useCallback(() => {
    api
      .listDocuments({ linked_entity_type: entityType, linked_entity_id: entityId })
      .then(setDocs)
      .catch(() => setDocs([]));
  }, [entityType, entityId]);
  useEffect(load, [load]);
