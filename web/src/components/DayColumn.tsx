...
export function DayColumn({
  ...
  date: Date;
  sessions: Session[];
  labelsFor: (s: Session) => CardLabels;
  canAdd: boolean;
  ...
  return (
    <div className="flex min-h-[60vh] w-56 shrink-0 flex-col rounded-lg bg-neutral-100/60">
      <div className="sticky top-0 z-[1] rounded-t-lg border-b border-neutral-200 bg-neutral-100 px-3 py-2 text-sm font-semibold text-neutral-700">
        {formatDay(date)}
        <span className="ml-1 font-normal text-neutral-400">({sessions.length})</span>
      </div>
      <div className="flex flex-1 flex-col gap-2 p-2">
        {sessions.map((s) => (
          <SessionCard key={s.id} session={s} labels={labelsFor(s)} onClick={() => onOpen(s.id)} />
        ))}
        {canAdd && (
          <button
            onClick={() => onAdd(date)}
